...
void test_nn_update(void) {
    ...
    TEST_ASSERT(list_head(R[0]) == NULL);

    TEST_ASSERT(list_remove(R[1],2) == 1);
    TEST_ASSERT(list_head(R[1]) == NULL);
    
    TEST_ASSERT(list_remove(R[2],3) == 1);
    TEST_ASSERT(list_head(R[2]) == NULL);

    TEST_ASSERT(list_head(R[3]) == NULL);

    TEST_ASSERT(list_remove(R[4],0) == 1);
    TEST_ASSERT(list_remove(R[4],3) == 1);
    TEST_ASSERT(list_head(R[4]) == NULL);

    nn_update(heaps, 2, 3, 9.4, R);

    TEST_ASSERT(list_remove(R[2],4) == 1);
    TEST_ASSERT(list_head(R[2]) == NULL);
